...
class Leak(Base):
    ...
    def ina(self, v: float) -> float:
        return self.g * (-60.95 +100.0) / (0.0 +100.0)*0.44*(v-55.0)
    
    def ica(self, v: float) -> float:
        return self.g * (-60.95 +100.0) / (0.0 +100.0)*0.25* (v -120.0)
   
    def ik(self, v: float) -> float:
        return (self.g * (-60.95 +100.0) / (0.0 +100.0)*0.31* (v +100.0))+(self.g * (-60.95-0) / (-100.0-0)*(v +100.0))
...
class LeakNana(Base):
    ...
    def __init__(self, g: Optional[int]=None, e: float=params.vNa) -> None:
        super().__init__(g, e)
...
class LeakNaca(Base):
    def __init__(self, g: Optional[int]=None, e: float=params.vCa) -> None:
        super().__init__(g, e)
...
class NaK(Base):
    ...
    def apump(self, v: float, na: float, k: float) ->float:
        x=1+(3.5/k)
        y=1+(10.0/na)
        z1=x**-2
        z2=y**-3
    ...
    def i(self, v: float, na: float, k: float) ->float:
        return self.g * self.apump(v,na,k)
